...
| `gl` | Peek the line in full — or, in a markdown table, the table as a grid |
| `zw` | Toggle soft wrap for the whole diff |
...
| `Ctrl+g` | Pick a commit to scope the diff to (`Esc` in the diff leaves it) |
...
| `gd` `gD` | Diff this file's two versions in `$EDITOR`, in a tmux pane |
