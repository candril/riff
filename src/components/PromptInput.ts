 ...
 * what this replaces.
 *
 * One instance, module-level: only one prompt is open at a time, and a
 * renderable that outlives the render pass is what keeps the cursor where
...
function ensureInput(renderer: CliRenderer): InputRenderable {
  if (!input) {
...
export interface PromptSession {
  ...
  /** Identifies the prompt. A new key re-seeds the field and takes focus. */
  key: string
...
export function syncPromptSession(session: PromptSession): void {
  const field = ensureInput(session.renderer)
...
/**
 * The field itself, to be placed in whatever is drawing the prompt. Every
 * caller gets the same renderable — it moves to the prompt that is open.
 */
export function PromptInput(renderer: CliRenderer): InputRenderable {
  return ensureInput(renderer)
}
