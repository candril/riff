...
# riff installer
# Usage: curl -fsSL https://raw.githubusercontent.com/candril/riff/main/scripts/install.sh | bash
#
#   RIFF_VERSION=0.2.0              install a specific release instead of the latest
#   RIFF_INSTALL_DIR=~/.local/bin   install somewhere other than /usr/local/bin
#
# Homebrew and Nix users have shorter routes:
#   brew install candril/tap/riff
#   nix run github:candril/riff
#
# This file is generated from candril/homebrew-tap/templates/install.sh; edit it there.

REPO="candril/riff"
INSTALL_DIR="${RIFF_INSTALL_DIR:-/usr/local/bin}"
BINARY_NAME="riff"
...
if [ -w "$INSTALL_DIR" ]; then
  mv "riff-${OS}-${ARCH}" "${INSTALL_DIR}/${BINARY_NAME}"
else
  echo "  ${INSTALL_DIR} is not writable — using sudo"
  sudo mv "riff-${OS}-${ARCH}" "${INSTALL_DIR}/${BINARY_NAME}"
fi

echo
echo "riff ${TAG} installed to ${INSTALL_DIR}/${BINARY_NAME}"
if ! command -v riff >/dev/null 2>&1; then
  echo "Note: ${INSTALL_DIR} is not on your PATH."
fi
echo "Run 'riff --help' to get started."
